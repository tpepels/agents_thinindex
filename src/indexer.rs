...
pub fn build_index(start: &Path) -> Result<BuildStats> {
    ...
    if !changed_files.is_empty() {
        ...
        for (path, rel, meta) in changed_files {
            ...
            let mut native_records = parser.parse_file(&path, &rel, &text);
...
/// without parsing files or rewriting the index. Returns `Ok(false)` if
...
/// at most 8KB) and avoids feeding non-text blobs to `read_to_string`,
/// which would otherwise abort the whole build.
