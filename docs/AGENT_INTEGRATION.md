...
`wi-init` does not modify global Codex, Cursor, GitHub Copilot, Claude, or OpenCode configuration. Use `wi-init --dry-run` to preview repo-local instruction file changes and the planned repo-local index build without writing files. The generated files are local, inspectable Markdown files, and `git diff` shows the exact instructions before commit.

thinindex does not currently bundle an MCP server, MCP helper command, or MCP client configuration helper. `wi-init` does not generate MCP client config, and no installed thinindex command should be documented as an MCP server today. MCP remains explicitly deferred until there is a local-only handler with bounded outputs, repo path validation, stale-index behavior matching `wi`, and no arbitrary shell execution.
...
- `mcp/README.md`: Explicit deferral note and local-only requirements for a possible future MCP wrapper; advisory documentation only, not generated client config.
