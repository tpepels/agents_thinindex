...
thinindex does not currently bundle an MCP server or MCP client configuration helper. MCP remains explicitly deferred until there is a local-only handler with bounded outputs, repo path validation, stale-index behavior matching `wi`, and no arbitrary shell execution.
...
- `mcp/README.md`: Explicit deferral note and local-only requirements for a possible future MCP wrapper.
