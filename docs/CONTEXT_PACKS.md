...
Pack output is built from SQLite `records`, `refs`, `dependencies`, `file_references`, and the same evidence model used by `wi impact`. Semantic facts are stored separately as an internal/deferred adapter boundary and are not consumed by baseline pack output. Every row includes a reason and confidence label.
...
- `Dependencies`: files imported or directly referenced by the primary definition files, including local assets/config/doc paths when useful.
- `Dependents`: files that import or otherwise reference the primary definition files.
- `Tests`: test-path references, test dependency edges, and same-name test conventions.
- `Configs/build files`: config, package/build, route, schema, JSON, TOML, YAML, and workflow references.
- `Docs/examples`: documentation links plus fixture/example files when they are relevant but lower confidence.
- `Unresolved hints`: unresolved dependency or file-reference evidence and other unknown areas that may matter.
