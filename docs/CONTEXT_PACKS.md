...
Pack output is built from SQLite `records`, `refs`, `dependencies`, `file_references`, and the same evidence model used by `wi impact`. Optional semantic facts are stored separately and are not required for baseline pack output. Every row includes a reason and confidence label.
