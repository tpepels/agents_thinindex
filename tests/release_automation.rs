...
#[test]
fn package_content_check_rejects_malformed_sbom() {
    let archive = make_archive(&["bad-sbom"]);

    Command::new(repo_root().join("scripts/check-package-contents"))
        .arg(&archive)
        .current_dir(repo_root())
        .assert_failure("expected archive with malformed SBOM.md to fail content check");
}

#[test]
fn package_content_check_rejects_unexpected_payload_entries() {
    let archive = make_archive(&["source-entry"]);

    Command::new(repo_root().join("scripts/check-package-contents"))
        .arg(&archive)
        .current_dir(repo_root())
        .assert_failure(
            "expected archive containing source checkout entries to fail content check",
        );
}
...
fn make_archive(options: &[&str]) -> PathBuf {
    ...
    let sbom = if options.contains(&"bad-sbom") {
        "sbom".to_string()
    } else {
        valid_sbom(&archive_name, "")
    };
    ...
    if options.contains(&"source-entry") {
        fs::create_dir_all(package.join("src")).expect("create src");
        fs::write(package.join("src/main.rs"), "fn main() {}").expect("write source");
    }
...
fn valid_sbom(archive_name: &str, exe_suffix: &str) -> String {
    format!(
        r#"# thinindex release SBOM

This is a compact source-controlled release SBOM generated by
`scripts/package-release`. It is intended to identify the shipped archive
payload and point to the authoritative dependency notices.
...
`THIRD_PARTY_NOTICES` is included in this archive and records bundled runtime
dependencies, parser grammar notices, generated parser source status, and the
optional external comparator boundary.

## Not bundled

- optional external comparator binaries
- signing keys, certificates, provisioning profiles, notarization credentials,
  or package-manager signing material
- repo-local indexes, quality reports, test repositories, build output, dist
  output, local benchmark output, or the source checkout
"#
    )
}
