...
pub fn assert_no_removed_parser_source(name: &str, records: &[IndexRecord]) {
    let removed_source = ["c", "tags"].concat();

    for rec in records {
        assert_ne!(
            rec.source, removed_source,
            "[{name}] record used removed parser source: {rec:?}"
        );
    }
}
...
pub fn run_named_index_integrity_checks(
    ...
    assert_no_removed_parser_source(name, &snapshot.records);
