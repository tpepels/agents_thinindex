...
Do not implement this until `PLAN_47_RELEASE_DISTRIBUTION_COMPLETION.md` is complete and green.

## Prerequisite

Plan 47 must be complete and committed. Treat the current release/distribution
state as the source of truth. Do not modify
`PLAN_47_RELEASE_DISTRIBUTION_COMPLETION.md` while executing this plan unless a
later human request explicitly asks for that file to change.

## Purpose

Complete the archive-focused release-distribution follow-up after Plan 47.
...
continues the existing local archive workflow without expanding into native
packages, signing, publishing, or update channels.
...
Selected slice for execution:

- Exact archive payload and generated `SBOM.md` manifest validation in
  `scripts/check-package-contents`.
- This builds on Plan 47's checksum and unpack smoke checks without adding
  native package formats, signing, publishing, update channels, parser changes,
  hosted behavior, telemetry, payments, or license enforcement.
...
- [x] Not applicable: `cargo test --test local_index -- --ignored` was not required because this plan did not change real-repo, parser, quality, refs, pack, impact, or dependency assumptions.
- [x] Not applicable: `cargo test --test real_repos -- --ignored` was not required because this plan did not change real-repo, parser, quality, refs, pack, impact, or dependency assumptions.
